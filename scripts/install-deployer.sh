...
FRONTEND="* * * * * /usr/bin/flock -n /tmp/x056-deploy.lock $DIR/scripts/frontend-deployer.sh"
( crontab -l 2>/dev/null | grep -vF "$DIR/scripts/deployer.sh" | grep -vF "$DIR/scripts/frontend-deployer.sh" || true; echo "$LINE"; echo "$FRONTEND" ) | crontab -
mkdir -p "$DIR/.deploy"
