...
- **Host escape hatch: `ssh valbox`** (aliases `legacy`, `103.30.246.154`; dedicated key in `state/ssh/`). This reaches the **actual host machine** that runs this gateway, as user `efran`, who is in the `docker` group — so from there you have the host's **real** Docker (all 90+ host containers, the host nginx edge in `/etc/nginx/sites-enabled/`, every project's stack). Use it for genuine host infra work (repoint the edge proxy, freeze/inspect a sibling service, manage another stack). Caveats: `efran` has no **passwordless** sudo, so host-OS changes (`systemctl reload nginx`, editing root-owned `/etc/nginx/…`) need a password you don't have — prepare the change and hand it to the user, or do it via a root container if appropriate. This is NOT a sandbox: never disturb the `x056-remote-control-x056`/`-dind` containers or other projects' containers.
