...
A self-hosted **remote control for Claude Code** with **automatic failover across two or more Claude Max accounts**. A supervisor drives headless `claude -p` sessions. When the active account reaches its usage limit, it resumes the same session through the next available account. A NestJS gateway + a single-file browser panel expose it anywhere: parallel projects, multiple concurrent conversations, live activity, a prompt queue, passkey login, and a mobile PWA.
...
- **Live activity + notifications**: see each conversation’s tool activity, subagent count, model, and account. A sidebar bell marks completion, failures, and questions.
...
**Projects & conversations.** The **+** next to "Projects" in the sidebar adds a repo from your workspace. Click a project to switch to it; its conversations list expands underneath (rename with Rename, remove with ×, or start a fresh one with **+ New conversation**). Conversations run concurrently within and across projects. Each active conversation has a sidebar spinner. The strip above the composer links to other active conversations.

**Sending messages.** Type in the composer and choose a model and effort. Attach files with the paperclip, paste, or drag and drop. Press Enter to send; Shift+Enter adds a newline. Each conversation keeps its **own** unsent draft, so switching chats never carries text over. Messages to an active conversation enter its queue. Edit or cancel them from the queue bar. Messages to an idle conversation start immediately.

**Accounts.** The account control shows which account the next message will use. Open it to inspect quotas, select an account, or add another account. Account setup provides a sign-in link and code entry. Force-switch moves an active turn to another eligible account.
...
The MCP exposes 45 tools for conversations, activity, artifacts, scheduling, queues, memory and code search.
...
Then point your TLS reverse proxy at `127.0.0.1:4056` and set `X056_RP_ID` (in `.env`) to that public hostname. Open the panel, enter the token once, then register a passkey (passkey icon in the top bar) for Face ID / Touch ID logins.
