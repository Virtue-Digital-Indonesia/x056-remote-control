...
window.createControlRoom = function (engine) {
  ...
  function setStageHidden(hidden,save=true){
    ...
    $('stageMode')?.querySelectorAll('button').forEach(b=>b.disabled=hidden);
    renderStage();
  }
  ...
  function renderStage(){
    ...
    const html=rows.map(x=>{
      ...
      return `<div class="stage-item ${current?'current':''}"><${window.rcProjectSpaces?.enabled()?'a href="'+window.rcProjectSpaces.conversationPath(x.p,x.c.sessionId)+'"':'button'} class="stage-conversation ${x.status} ${x.unread?'unread':''}" data-chat-state="${esc(x.status)}" data-project="${esc(x.p.id)}" data-session="${esc(x.c.sessionId)}" aria-label="${esc(primary+' · '+secondary+' · '+(current?'Viewing · ':'')+(x.unread?'Unread · ':'')+status)}" ${current?'aria-current="true"':''}><span class="stage-caption"><strong>${esc(primary)}</strong><small>${esc(secondary)}</small><span class="stage-caption-status">${x.unread?'Unread · ':''}${stageMode==='smart'&&isStagePinned(x.p.id,x.c.sessionId)?'Pinned · ':''}${status}</span></span>${ic(statusGlyph(x.status))}<i class="stage-status" aria-hidden="true"></i></${window.rcProjectSpaces?.enabled()?'a':'button'}><button class="stage-unpin" data-stage-unpin="${esc(stageKey(x.p.id,x.c.sessionId))}" aria-label="${stageMode==='pinned'?'Unpin':'Dismiss'} ${esc(title)}">${ic('x')}</button></div>`;
    }).join('');
    ...
    $('stageItems').querySelectorAll('[data-session]').forEach(b=>b.onclick=e=>{if(e.metaKey||e.ctrlKey||e.shiftKey||e.altKey)return;e.preventDefault();openConversation(b);});
  ...
  function showSection(next,preserveRoute=false) {
    ...
    if(!window.rcWorkspace?.ready())$('crBreadcrumb').innerHTML='Workspace<span class="crumb-sep">/</span><span>'+({board:'Control room',accounts:'Dashboard',automations:'Automations',artifacts:'Artifact library',planner:'Queue planner',memory:'Memory'}[next])+'</span>';
  ...
  document.addEventListener('keydown', e => {
    ...
    if (!window.rcWorkspace?.ready() && e.key === 'Escape' && shell.classList.contains('projects-open')) { e.preventDefault();projectNav(false);$('crProjects').focus();return; }
    if (!window.rcWorkspace?.ready() && e.key === 'Tab' && shell.classList.contains('projects-open') && innerWidth<=850) {
      const nodes=[$('crProjects'),...$('crProjectNav').querySelectorAll('button,input,a')].filter(n=>!n.disabled),first=nodes[0],last=nodes[nodes.length-1];
  ...
  function renderProjectNav(all,state) {
    ...
    const html=`<${spaces?'a href="/projects" data-space-link':'button'} class="cr-project-link ${navSelection===''?'selected':''}" data-scope="" aria-current="${navSelection===''?'page':'false'}">${ic('menu')}<span>All projects</span><small>${all.filter(x=>x.p.kind!=='chat'&&(showDismissedProjects||!dismissedProjects.includes(x.p.id))).length}</small></${spaces?'a':'button'}><div class="cr-project-separator"></div>`+state.projects.filter(p=>p.kind!=='chat'&&(showDismissedProjects||!dismissedProjects.includes(p.id))&&p.name.toLowerCase().includes(query)).map(p=>{
      ...
      return `<${spaces?'a href="/projects/'+encodeURIComponent(p.id)+'/chat" data-space-link':'button'} class="cr-project-link ${p.id===navSelection?'selected':''} ${dismissedProjects.includes(p.id)?'dismissed':''}" data-scope="${esc(p.id)}" aria-current="${p.id===navSelection?'page':'false'}" title="${esc(p.name)}${dismissedProjects.includes(p.id)?' · Dismissed':''}">${ic('folder')}<span>${esc(p.name)}</span>${attention?`<small class="attention" title="${attention} need attention">${attention}</small>`:running?`<small class="working" title="${running} running">${running}</small>`:`<small>${rows.length}</small>`}</${spaces?'a':'button'}>`;
    }).join('');
    ...
    if(markup!==projectNavSignature){projectNavSignature=markup;$('crProjectLinks').innerHTML=markup;$('crProjectLinks').querySelectorAll('[data-scope]').forEach(b=>b.onclick=()=>{if(!spaces)selectProjectScope(b.dataset.scope);});if($('crDismissedProjects'))$('crDismissedProjects').onclick=()=>{showDismissedProjects=!showDismissedProjects;refresh();};}
  }
  ...
  async function openConversation(button) {
    if (window.rcProjectSpaces?.enabled()) return window.rcProjectSpaces.openConversation(button.dataset.project, button.dataset.session);
  ...
  function conversationRow(x) {
    const routed = window.rcProjectSpaces?.enabled(), rowTag = routed ? 'a' : 'button', rowLink = routed ? ' href="'+esc(window.rcProjectSpaces.conversationPath(x.p, x.c.sessionId))+'"' : '';
    const [primary,secondary]=conversationLabels(x.p,x.c);
    return `<article class="cr-conversation-row ${x.status}" data-row="${esc(x.c.sessionId)}">${bulkMode?`<input class="bulk-checkbox" type="checkbox" data-bulk-key="${esc(x.k)}" aria-label="Select ${esc(x.c.title||'conversation')}" ${bulkSelection.has(x.k)?'checked':''}>`:''}<${rowTag}${rowLink} class="cr-task" data-project="${esc(x.p.id)}" data-session="${esc(x.c.sessionId)}">${stateDot(x.status)}<span class="cr-row-copy"><span class="cr-row-title" title="${esc(primary)}">${esc(primary)}<small class="cr-row-tags">${esc((conversationMeta[x.k]?.tags||[]).join(' · '))}</small>${x.draft?'<small class="cr-draft-tag">Draft</small>':''}${x.unread?'<i class="cr-unread-dot" title="Unread"></i>':''}</span><span class="cr-row-subtitle" title="${esc(secondary)}">${esc(secondary)}</span></span><span class="cr-status ${x.status}"><i></i>${statusLabels[x.status]}</span><time ${x.time?'datetime="'+new Date(x.time).toISOString()+'" title="Last message: '+esc(new Date(x.time).toLocaleString())+'"':'title="No message timestamp available"'}>${esc(relativeDate(x.time))}</time></${rowTag}><button class="cr-row-menu cr-icon" data-conversation-menu="${esc(x.c.sessionId)}" data-project="${esc(x.p.id)}" aria-label="Actions for ${esc(x.c.title||'conversation')}">${ic('more')}</button><button class="cr-row-pin ${isStagePinned(x.p.id,x.c.sessionId)?'pinned':''}" data-pin-project="${esc(x.p.id)}" data-pin-session="${esc(x.c.sessionId)}" aria-label="${isStagePinned(x.p.id,x.c.sessionId)?'Unpin':'Pin'} ${esc(x.c.title||'conversation')}" aria-pressed="${isStagePinned(x.p.id,x.c.sessionId)}">${ic('pin')}</button>${['finished','idle'].includes(x.status)&&!x.draft?`<button class="cr-recent-dismiss" data-recent-project="${esc(x.p.id)}" data-recent-session="${esc(x.c.sessionId)}" aria-label="${recentDismissed.includes(stageKey(x.p.id,x.c.sessionId))?'Restore':'Dismiss'} ${esc(x.c.title||'conversation')} ${recentDismissed.includes(stageKey(x.p.id,x.c.sessionId))?'to':'from'} recents" title="${recentDismissed.includes(stageKey(x.p.id,x.c.sessionId))?'Restore to recents':'Dismiss from recents'}">${ic(recentDismissed.includes(stageKey(x.p.id,x.c.sessionId))?'history':'x')}</button>`:''}${x.status==='question'?`<button class="cr-dismiss" data-dismiss="${esc(x.c.sessionId)}" data-project="${esc(x.p.id)}" title="Dismiss question without sending a reply">Dismiss question</button>`:''}${x.status==='failed'?`<button class="cr-dismiss" data-dismiss-failed="${esc(x.c.sessionId)}" data-project="${esc(x.p.id)}" title="Clear the failed mark. The conversation and its history stay.">Dismiss failure</button>`:''}</article>`;
  }
  function renderBoard() {
    ...
    const draftOnly=boardFilter==='draft';
    const unread=draftOnly?[]:filtered.filter(x=>x.unread),attention=draftOnly?[]:filtered.filter(x=>!x.unread&&['question','failed','parked'].includes(x.status)),active=draftOnly?[]:filtered.filter(x=>!x.unread&&['running','background'].includes(x.status));
    const drafts=filtered.filter(x=>x.draft&&(draftOnly||!x.unread&&!['question','failed','parked','running','background'].includes(x.status)));
    const recentCandidates=filtered.filter(x=>!x.draft&&!x.unread&&['finished','idle'].includes(x.status)&&(!recentDismissed.includes(stageKey(x.p.id,x.c.sessionId))||query||showDismissedRecents||boardFilter!=='all'));
    ...
    if(html!==boardSignature){
      ...
      $('crLanes').querySelectorAll('.cr-task').forEach(b=>b.onclick=e=>{if(b.tagName==='A'&&(e.metaKey||e.ctrlKey||e.shiftKey||e.altKey||e.button!==0))return;e.preventDefault();openConversation(b);});
      ...
      $('crLanes').querySelectorAll('[data-dismiss-failed]').forEach(b=>b.onclick=async()=>{b.disabled=true;const pid=b.dataset.project,sid=b.dataset.dismissFailed;try{await json('/api/conversations/outcome/dismiss',{projectId:pid,sessionId:sid});outcomes.delete(pid+'::'+sid);engine.setConversationUnread(pid,sid,false);toast('Failure dismissed. The conversation stays.');refresh();}catch(e){toast(e.message);b.disabled=false;}});
  ...
  function costRows(){
    if(window.rcProjectSpaces?.enabled()&&costSnapshot?.projectSpaces)return costSnapshot.projectSpaces;
    const projects=engine.state().projects, rows=costSnapshot?.projects||[], chats=rows.filter(row=>projects.find(p=>p.id===row.projectId)?.kind==='chat');
    if(!chats.length)return rows;
    const grouped={...chats[0],projectId:'__chats__',projectName:'Chat',projectIds:chats.map(p=>p.projectId),usage:{},cost:{usd:0,unpriced:[]}};
    for(const key of ['conversations','agentCount','agentUsd','missing','unstarted'])grouped[key]=chats.reduce((n,p)=>n+(p[key]||0),0);
    for(const key of ['input','output','cacheRead','cacheWrite'])grouped.usage[key]=chats.reduce((n,p)=>n+(p.usage[key]||0),0);
    grouped.cost.usd=chats.reduce((n,p)=>n+p.cost.usd,0);grouped.cost.unpriced=[...new Set(chats.flatMap(p=>p.cost.unpriced))];grouped.partial=chats.some(p=>p.partial);
    return [...rows.filter(p=>!grouped.projectIds.includes(p.projectId)),grouped];
  }
  function renderProjectCosts(){
    ...
    const partial=rows.some(p=>p.partial),unknown=[...new Set(rows.flatMap(p=>p.cost.unpriced))],missing=rows.reduce((n,p)=>n+p.missing,0),unstarted=rows.reduce((n,p)=>n+(p.unstarted||0),0);
    const markup=`<div><span class="cr-eyebrow">PROJECT COST ESTIMATE</span><div class="cr-cost-value">${costSnapshot?'≈ '+money(usd):'—'}${costSnapshot?'<span class="cr-cost-idr">≈ '+rupiah(usd)+'</span>':''}<small>${costSnapshot?compact(tokens)+' recorded tokens':'Loading recorded usage…'}</small></div><p>${costError?esc(costError):partial?'Scanning transcripts · totals are still updating':unknown.length?'Some models are unpriced':missing?'Some conversations are missing transcripts':unstarted?'Some conversations were never started':'At standard API rates · includes subagents'}</p></div><button class="cr-secondary" id="crCostDetails">${costError?'Retry':'View breakdown'} ${ic('right')}</button>`;
  ...
  function projectMenu(anchor,id,point){
    ...
    openMenu(anchor,[{label:'New conversation',icon:'plus',disabled:!!project.archivedAt,run:()=>engine.newConversation(id)},{label:'Rename project',icon:'edit',run:()=>engine.renameProject(id)},...(window.rcProjectSpaces?.enabled()&&project.kind!=='chat'?[{label:project.archivedAt?'Restore Work project':'Archive Work project',icon:'folder',run:async()=>{if(!await engine.confirm({title:project.archivedAt?'Restore Work project?':'Archive Work project?',message:project.archivedAt?'Retained conversations become available. Queues and automations still need review.':'Pause this repository’s conversations wherever they are organized. Chats and reference links remain independent.'}))return;try{await workspaceRequest('project-spaces/executions/'+encodeURIComponent(id)+'/archive',{expectedRevision:project.revision||0,operationId:crypto.randomUUID(),archived:!project.archivedAt});await engine.reloadProjects();refresh();}catch(e){toast(e.message);}}}]:[]),{label:dismissedProjects.includes(id)?'Restore project':'Dismiss project',icon:'x',run:()=>setProjectDismissed(id,!dismissedProjects.includes(id))}],point);
  }
  function rowConversationMenu(anchor,pid,sid,point){
    ...
    openMenu(anchor,[
      ...
      {label:'Rename conversation',icon:'edit',run:()=>engine.renameConversation(pid,sid)},
      ...
      {label:'Conversation results',icon:'results',run:()=>showResults({projectId:pid,sessionId:sid})},{label:'Routing & accounts',icon:'route',disabled:!sid,run:()=>showRouting({projectId:pid,sessionId:sid})},{label:'Continue with another provider',icon:'switch',disabled:!sid||engine.state().projects.find(p=>p.id===pid)?.kind==='chat',run:()=>showHandoff({projectId:pid,sessionId:sid})},{label:'Conversation memory',icon:'snippet',run:()=>showMemoryContext({projectId:pid,sessionId:sid})},{label:'Message delivery',icon:'send',run:engine.showDelivery},{label:'Copy conversation ID',icon:'copy',run:()=>navigator.clipboard.writeText(sid).then(()=>toast('Conversation ID copied.')).catch(()=>toast('Could not copy the conversation ID.'))},
      ...(running?[{label:'Stop turn',icon:'stop',run:async()=>{try{await json('/api/sessions/current/stop',{projectId:pid,sessionId:sid});toast('Stopping turn.');}catch(e){toast(e.message);}}}]:[]),
      {label:'Remove from panel',icon:'trash',disabled:running,run:()=>engine.removeConversation(pid,sid)},
    ],point);
  }
  ...
  async function showMemorySource(id) {
    ...
    try {
      ...
      d.querySelector('[data-origin]')?.addEventListener('click', async () => {
        d.close();
        if(window.rcProjectSpaces?.enabled()){d.close();await window.rcProjectSpaces.openConversation(s.projectId,s.sessionId);return;}
    ...
    async function load() {
      ...
      try {
        const query = d.querySelector('[data-context-query]').value,
          params = new URLSearchParams({ projectId: target.projectId, sessionId: target.sessionId, query,requestId:target.requestId||engine.memoryDraftRequest(target.projectId,target.sessionId) }),
      ...
      d.querySelector('[data-title-list]').innerHTML =
        filtered
          .map((x) => {
            ...
            const pending=x.status==='waiting'||x.status==='generating',pendingText=x.status==='generating'?'Generating title…':x.attempts?'Retry queued…':'Waiting to generate…';
            return `<article class="title-suggestion" data-title-job="${esc(x.id)}" data-status="${x.status}"><input type="checkbox" data-title-select="${esc(x.id)}" aria-label="Apply ${esc(x.title || x.before)}" ${selected.has(x.id) && x.status === 'ready' ? 'checked' : ''} ${x.status !== 'ready' ? 'disabled' : ''}><div class="title-suggestion-body"><div class="title-suggestion-meta"><span>${esc(project?.name || 'Project')} · ${x.provider === 'codex' ? 'ChatGPT' : 'Claude'}${x.beforeOrigin === 'manual' ? ' · Manually named' : ''}</span><span>${x.status==='waiting'&&x.attempts?'Retry queued':labels[x.status]}</span></div><div class="title-name-comparison ${pending?'pending':''}"><div><small>${x.status === 'applied' ? 'Previous title' : x.status === 'stale' ? 'Original title' : 'Current title'}</small><span>${esc(x.before)}</span></div><span class="title-name-arrow" aria-hidden="true">${ic('right')}</span><div><small>${pending?'Status':x.status === 'applied' ? 'Applied title' : 'Suggested title'}</small><strong>${esc(x.title || (pending ? pendingText : 'No suggestion'))}</strong></div></div>${x.reason ? `<p class="cr-note">${esc(x.reason)}</p>` : ''}<div class="title-suggestion-actions">${x.status === 'applied' ? `<button class="cr-text-button" data-undo-title="${x.id}">Undo rename</button>` : ['failed', 'skipped', 'stale'].includes(x.status) ? `<button class="cr-text-button" data-retry-title="${x.id}">Suggest again</button>` : ''}${['waiting', 'generating', 'ready'].includes(x.status) ? `<button class="cr-text-button" data-dismiss-title="${x.id}">Dismiss</button>` : ''}</div></div></article>`;
          })
          .join('') ||
  ...
  function queueReason(item,queues=plannerRows){
    ...
    const reasons=[];
    if(item.contextReview)reasons.push(item.contextReview.reason+' · Review required');
