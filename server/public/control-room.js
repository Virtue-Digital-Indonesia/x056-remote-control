...
window.createControlRoom = function (engine) {
  ...
  function renderProjectNav(all,state) {
    const query=$('crProjectSearch').value.toLowerCase(), spaces=window.rcProjectSpaces?.enabled();
    const html=`<button class="cr-project-link ${!selectedProject?'selected':''}" data-scope="" aria-current="${!selectedProject?'page':'false'}">${ic('menu')}<span>All projects</span><small>${all.filter(x=>x.p.kind!=='chat'&&(showDismissedProjects||!dismissedProjects.includes(x.p.id))).length}</small></button><div class="cr-project-separator"></div>`+state.projects.filter(p=>p.kind!=='chat'&&(showDismissedProjects||!dismissedProjects.includes(p.id))&&p.name.toLowerCase().includes(query)).map(p=>{
      ...
      return `<${spaces?'a href="/projects/'+encodeURIComponent(p.id)+'/chat" data-space-link':'button'} class="cr-project-link ${p.id===selectedProject?'selected':''} ${dismissedProjects.includes(p.id)?'dismissed':''}" data-scope="${esc(p.id)}" aria-current="${p.id===selectedProject?'page':'false'}" title="${esc(p.name)}${dismissedProjects.includes(p.id)?' · Dismissed':''}">${ic('folder')}<span>${esc(p.name)}</span>${attention?`<small class="attention" title="${attention} need attention">${attention}</small>`:running?`<small class="working" title="${running} running">${running}</small>`:`<small>${rows.length}</small>`}</${spaces?'a':'button'}>`;
    }).join('');
    ...
    if(markup!==projectNavSignature){projectNavSignature=markup;$('crProjectLinks').innerHTML=markup;$('crProjectLinks').querySelectorAll('[data-scope]').forEach(b=>b.onclick=()=>{if(!spaces||!b.dataset.scope)selectProjectScope(b.dataset.scope);});if($('crDismissedProjects'))$('crDismissedProjects').onclick=()=>{showDismissedProjects=!showDismissedProjects;refresh();};}
  }
