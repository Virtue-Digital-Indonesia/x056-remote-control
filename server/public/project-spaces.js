...
window.createProjectSpaces = function (engine, room, chat) {
  ...
  async function route() {
    ...
    try {
      ...
      const page=$('rcProjectPage'), wasShowing=!page.hidden&&document.body.classList.contains('rc-spaces-view');
      if(!wasShowing||room.isOpen())room.showBoard(); document.body.classList.add('rc-spaces-view'); page.hidden = false;
      $('crSpacesTab').setAttribute('aria-current', 'page'); $('crBoardTab').removeAttribute('aria-current');
      if(!wasShowing||!page.dataset.project)page.innerHTML = '<p class="cr-empty" role="status">Loading Project…</p>';
      await load(); if (generation !== version || location.pathname !== url) return;
      activeId = segments[1] || ''; activeTab = segments[2] || 'overview';if($('crBreadcrumb')&&!window.rcWorkspace?.ready())$('crBreadcrumb').innerHTML=link('/projects','Projects')+'<span class="crumb-sep">/</span><span>'+esc(projects.find(p=>p.id===activeId)?.name||'All projects')+'</span>';room.refresh();
      ...
      delete $('rcProjectPage').dataset.project;
  ...
  async function renderProject(p) {
    const page=$('rcProjectPage'), shellBuilt=!(page.dataset.project===p.id&&$('rcProjectBody'));
    if(!shellBuilt){
      page.querySelectorAll('.rc-space-tabs a').forEach(a=>{if(a.getAttribute('href')===path(p.id,activeTab))a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});
      restoreMemory(); $('rcProjectBody').innerHTML='';
    } else { page.dataset.project=p.id;
    page.innerHTML = `<div class="rc-space-breadcrumb">${link('/projects', 'Projects')}<span>/</span><span>${esc(p.name)}</span></div><div class="cr-heading"><div><h1 tabindex="-1">${esc(p.name)}</h1><p>${p.archivedAt ? 'Archived · saved history and files are retained' : `${p.activity.running} running · ${p.activity.background} background · ${p.activity.queued} queued · ${p.activity.needsInput} need input`}</p></div><div class="rc-space-toolbar"><button class="cr-secondary" id="rcSpaceAddExisting" ${p.archivedAt?'disabled':''}>${icon('plus')} Add existing</button>${link(path(p.id, 'settings'), icon('gear') + ' Settings', 'cr-secondary')}</div></div><nav class="rc-space-tabs" aria-label="Project sections">${['overview','chat', 'work', 'files', 'memory'].map(tab => `<a href="${path(p.id, tab)}" data-space-link ${tab === activeTab ? 'aria-current="page"' : ''}>${icon({overview:'layout',chat:'chat',work:'terminal',files:'file',memory:'snippet'}[tab])}${tab[0].toUpperCase() + tab.slice(1)}</a>`).join('')}</nav><div id="rcProjectBody"></div>`;
    ...
    if (activeTab === 'memory') {
      ...
      if(!window.rcWorkspace?.ready())$('crBreadcrumb').innerHTML=link('/projects','Projects')+'<span class="crumb-sep">/</span><span>'+esc(p.name)+'</span>';
    }
    if (activeTab === 'settings') renderSettings(p);
    const body=$('rcProjectBody');
    if(shellBuilt){window.rcMotion?.enter(page);}else if(body){window.rcMotion?.enter(body);}
    if(body)window.rcMotion?.list(body);
  }
  ...
  function overviewConversation(x){
    ...
    return link(conversationPath(x.p,x.c.sessionId),`${icon(x.p.kind==='chat'?'chat':'terminal')}<span><strong>${esc(x.c.title)}</strong><small>${x.p.kind==='chat'?'Chat':'Work · '+esc(x.p.name)} · ${esc(({codex:'Codex',claude:'Claude'})[x.c.provider||'claude']||'Provider unknown')}</small></span><small>${status}</small>`,'workspace-overview-row');
  }
  async function renderOverview(p){
    const host=$('rcProjectBody'),serial=generation,rows=overviewRows(p),resume=rows[0];
    host.innerHTML=`<div class="workspace-overview"><div><section class="workspace-resume"><div class="cr-eyebrow">PICK UP WHERE YOU LEFT OFF</div>${resume?`<span class="workspace-kind">${icon(resume.p.kind==='chat'?'chat':'terminal')}${resume.p.kind==='chat'?'Chat':'Work'}</span><h2>${esc(resume.c.title)}</h2><p>Continue with the same conversation, saved files, and Project context.</p><div id="rcOverviewRecentFile"></div><footer>${link(conversationPath(resume.p,resume.c.sessionId),'Continue '+(resume.p.kind==='chat'?'Chat':'Work')+' '+icon('right'),'cr-primary')}<small>${resume.time?'Last activity '+esc(new Date(resume.time).toLocaleDateString()):'Ready when you are'}</small></footer>`:`<h2>What are we working on?</h2><p>Start a Chat, or link an existing conversation and its history.</p><button class="cr-primary" data-overview-new="chat" ${p.archivedAt?'disabled':''}>${icon('plus')} New Chat</button>`}</section><div class="workspace-section-heading"><h2>Recent conversations</h2><div class="rc-space-toolbar" style="margin:0"><button class="cr-secondary" data-overview-new="chat" ${p.archivedAt?'disabled':''}>New Chat</button><button class="cr-secondary" data-overview-new="work" ${p.archivedAt?'disabled':''}>New Work</button></div></div><div class="workspace-overview-rows">${rows.slice(0,6).map(overviewConversation).join('')||'<p class="rc-space-note">Your conversations will appear here.</p>'}</div><div class="workspace-section-heading"><h2>Needs your attention</h2>${link(path(p.id,'memory'),'Open Memory')}</div><div id="rcOverviewAttention">${rows.filter(r=>['question','failed','parked'].includes(r.status)).map(overviewConversation).join('')}<p id="rcOverviewMemoryReview" class="rc-space-note" role="status">Checking memory proposals…</p></div></div><aside class="workspace-overview-aside"><section id="rcBriefCard"><h2>Project brief</h2><p id="rcOverviewBrief" role="status">Loading brief…</p><div id="rcBriefEditor" hidden><textarea id="rcBriefText" rows="7" maxlength="64000" aria-label="Project brief" placeholder="What this Project is, what matters, and what every conversation should know."></textarea><p role="alert"></p><footer><button class="cr-secondary" id="rcBriefCancel" type="button">Cancel</button><button class="cr-primary" id="rcBriefSave" type="button">Save brief</button></footer></div><button class="cr-secondary" id="rcOverviewEditBrief">Edit brief</button></section><section id="rcOverviewCost" class="workspace-cost" aria-label="Project cost estimate"><h2>Total Project cost estimate</h2><strong>—</strong><small role="status">Loading recorded usage…</small></section><section><h2>Work repositories</h2>${(p.workProjects||[]).map(w=>link('/work/'+encodeURIComponent(w.id),`${icon('terminal')}<span><strong>${esc(w.name)}</strong><small>${w.wholeProject?'Whole repository':'Selected conversations'}</small></span>`,'workspace-overview-row')).join('')||'<p>No Work repository linked yet.</p>'}<button class="cr-secondary" id="rcOverviewAddWork" ${p.archivedAt?'disabled':''}>Add workspace</button></section><section><h2>Project Files</h2><p id="rcOverviewFileCount" role="status">Loading saved files…</p>${link(path(p.id,'files'),'Browse files '+icon('right'),'cr-secondary')}</section></aside></div>`;
    host.querySelectorAll('[data-overview-new]').forEach(b=>b.onclick=()=>newConversation(p,b.dataset.overviewNew));
    $('rcOverviewAddWork').onclick=()=>addWorkProject(p);
    let briefEntry=null;
    const briefOpen=open=>{$('rcBriefEditor').hidden=!open;$('rcOverviewBrief').hidden=open;$('rcOverviewEditBrief').hidden=open;if(open){$('rcBriefText').value=briefEntry?.content||'';$('rcBriefText').focus();}};
    $('rcOverviewEditBrief').onclick=()=>briefOpen(true);$('rcBriefCancel').onclick=()=>briefOpen(false);
    $('rcBriefSave').onclick=async()=>{const content=$('rcBriefText').value.trim(),alert=$('rcBriefEditor').querySelector('[role=alert]');$('rcBriefSave').disabled=true;alert.textContent='';
      try{if(!content)throw new Error('Write the brief first.');
        const saved=await request('/api/memory/entry',{id:briefEntry?.id,revision:briefEntry?.revision,entry:{title:p.name+' brief',content,kind:'context',scope:'space',spaceId:p.id,status:'confirmed',pinned:true,tags:['project-brief'],providers:['claude','codex']}});
        briefEntry=saved;$('rcOverviewBrief').textContent=saved.content;$('rcOverviewEditBrief').textContent='Edit brief';briefOpen(false);room.notify('Brief saved. Every conversation in this Project starts from it.');}
      catch(e){alert.textContent=e.message;}finally{$('rcBriefSave').disabled=false;}};
    ...
    briefEntry=brief.status==='fulfilled'?(brief.value.items.find(e=>(e.spaceId||e.projectId)===p.id&&e.status==='confirmed')||null):null;
    $('rcOverviewBrief').textContent=brief.status==='fulfilled'?(briefEntry?.content||'No brief yet. Write a few lines so every conversation starts from the same Project context.'):'Could not load the brief. Refresh to retry.';
    $('rcOverviewEditBrief').textContent=briefEntry?'Edit brief':'Write the brief';
  ...
  function newConversation(p, mode, workId) {
    if(mode==='work'&&!workId&&!p?.workspaceConfigured)return createWorkspace(p);
    ...
    const d=dialog('New '+mode,`<form class="workspace-form">${mode==='work'?`<label>Name<input name="name" placeholder="New work" maxlength="300"></label>${workChoices(chosen)}`:'<p class="rc-space-note">The chat is named from your first message.</p>'}${controlsMarkup}<p role="alert"></p><button class="cr-primary">Create ${mode}</button></form>`),form=d.querySelector('form'),requestId=crypto.randomUUID();
  ...
  function addWorkProject(p) {
    const chooser=dialog('Add Work workspace','<div class="workspace-form"><button class="cr-primary" data-create>Create a new folder and Git repository</button><button class="cr-secondary" data-existing>Use an existing workspace</button></div>');
    chooser.querySelector('[data-create]').onclick=()=>{chooser.close();createWorkspace(p);};
  ...
  return {
    ...
    conversationItems:()=>{
      ...
      if(isChat)items.push({label:'Start a fresh linked Chat',icon:'message-plus',run:()=>handoff('chat',true)});
      items.push({label:'Change Project',icon:'move',run:()=>membershipDialog(source,undefined,isChat?undefined:sid)});
      for(const l of handoffLinks)items.push({label:l.label,icon:'link',run:()=>navigate(l.path)});
      return items;
    },
