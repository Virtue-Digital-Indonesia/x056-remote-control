...
window.createRCChat = function (engine, room) {
  ...
  let enabled = false, mounted = false, active = '', selectedFile = '', selectedVersion = '', tab = 'files', files = [], sequence = 0, previewTimer, objectURLs = [], showArchived = false;
  const jobs = new Map();
  ...
  function renderPanel() {
    ...
    $('rcChatPanelBody').innerHTML = `<div class="rc-chat-panel-heading"><span>${files.length} files</span><button id="rcChatUploadButton">+ Upload</button></div>` + files.map(file => {
      const v = file.versions.find(v => v.id === file.latestVersionId);
      return `<article class="rc-chat-file" data-file="${file.id}"><button data-preview><span class="rc-chat-file-icon">${esc(file.name.split('.').pop().slice(0,5).toUpperCase())}</span><strong>${esc(file.name)}</strong></button><small>v${file.versions.length} · ${Math.max(1,Math.round(v.bytes/1024))} KB · Saved</small><div><button data-attach>Attach</button><button data-download>Download</button><button data-history>Versions</button></div></article>`;
    }).join('') + (!files.length ? '<div class="rc-chat-empty"><strong>Bring your work here</strong><p>Upload a proposal, PDF, or reference file. Saved versions stay with this chat across accounts.</p></div>' : '');
  ...
  async function renderPreview() {
    ...
    async function blobImage(path, host) { const r=await engine.api(path);if(!r.ok)throw new Error('Preview unavailable');const blob=await r.blob();if(serial!==sequence)return;const url=URL.createObjectURL(blob);objectURLs.push(url);const img=document.createElement('img');img.src=url;img.alt=file.name+' preview';host.replaceChildren(img); }
    try {
