...
const CONTAINER_SYSTEM_NOTE =
  ...
  '(4) `docker` and `docker compose` DO work, but they drive an isolated Docker-in-Docker sidecar (DOCKER_HOST=tcp://dind:2375), NOT the host Docker — use them for your project\'s own builds/e2e; you still cannot touch host containers or deploy the gateway (deploys go through the host actuator: commit + `touch .deploy/requested`). Two caveats for a project compose: bind mounts resolve on the dind daemon (which shares the workspace at the same path /home/efran/remote-development, so mounts under the workspace work), and published ports are reachable at hostname `dind:<port>`, not localhost. If a git push over an SSH remote fails on authentication, the credential for that remote is not configured — surface it to the user instead of retrying. ' +
  'Toolchains you DO have: Node/npm, Go (GOTOOLCHAIN=auto), Java 17 + Maven, Python 3 (create a venv — the system Python is externally-managed), PHP + Composer, gcc/make, git, ripgrep, and a headless Chromium — screenshot any local URL with `node /app/scripts/shot.cjs <url> <out.png>` then read the PNG. git push over SSH to GitHub and to the VPN host 192.168.83.20 both work.';
