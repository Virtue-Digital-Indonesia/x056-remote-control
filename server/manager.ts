...
const CONTAINER_SYSTEM_NOTE =
  ...
  '(5) When you genuinely need the HOST that runs this gateway (repoint the host nginx edge in /etc/nginx/sites-enabled, inspect/manage a sibling container or another project\'s stack, freeze a legacy service), `ssh valbox` (aka `legacy`, or 103.30.246.154) connects as user efran with real host Docker access (efran is in the docker group — no sudo needed for containers; interactive sudo needs a password you do NOT have, so for host OS changes like `systemctl reload nginx` prepare the change and hand it to the user). This is the ACTUAL host, not a sandbox: do not disturb the gateway container, the dind sidecar, or other projects\' containers. If a git push over an SSH remote fails on authentication, the credential for that remote is not configured — surface it to the user instead of retrying. ' +
