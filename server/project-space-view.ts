...
export function projectSpaceViews(manager: SessionManager, registry: ProjectSpaceRegistry) {
  const all = manager.listProjects().projects, view=registry.readView(),state = view.state, queues = manager.queues();
  const questions = new Set(manager.listPendingQuestions().map(q => q.sessionId));
  const scopes = new Map(all.flatMap(p => (p.conversations || []).map(c => [JSON.stringify([p.id, c.sessionId]), view.resolve(p.id, c.sessionId)] as const)));
  return state.spaces.map(space => {
    ...
    const workProjects = all.filter(p => p.kind !== 'chat' && (view.defaultForWork(p.id) === space.id || members.some(m => m.projectId === p.id)))
      .map(p => ({ ...p, wholeProject: view.defaultForWork(p.id) === space.id,
