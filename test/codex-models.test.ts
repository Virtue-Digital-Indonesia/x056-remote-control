...
describe('codex listModels across the fleet', () => {
  ...
  it('offers a model any account has, ordered by the freshest catalog', () => {
    ...
    expect(slugs).toEqual(['gpt-6-astra', 'gpt-6-sol', 'gpt-5.5', 'gpt-6-luna']);
  });
  ...
  it('skips hidden entries and accounts with no cache yet', () => {
    ...
    expect(codexAdapter.listModels!([never, one]).map((m) => m.slug)).toEqual(['gpt-5.5', 'gpt-6-sol', 'gpt-6-luna']);
    expect(codexAdapter.listModels!([never]).map(m => m.slug)).toEqual(['gpt-6-sol', 'gpt-6-luna']);
  });
});
...
it('retired caches cannot reintroduce GPT-5.6 Sol or Luna', () => {
  ...
  expect(models.map(m => m.slug)).toEqual(['gpt-5.6-terra', 'gpt-6-sol', 'gpt-6-luna']);
  expect(models[1].efforts).toEqual(['low', 'medium', 'high', 'xhigh', 'max']);
});
