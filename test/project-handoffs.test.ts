...
function fixture() {
  ...
  const p = m.createProjectSpace({ requestId: 'handoff-parent-001', name: 'Proposal' });
  const work = m.createProject('Work repository', root); associateWork(m, work.id, p.id);
...
describe('Project Chat and Work handoffs', () => {
  ...
  it('reuses one target and initial message with exact memory and file versions', async () => {
    ...
    const memory = f.m.memory().create({ projectId: f.chat.id, scope: 'project', status: 'confirmed', title: 'Audience', content: 'Write for the review committee.', pinned: true }, 'operator');
    ...
    expect(first.target).toEqual(again.target); expect(f.m.listConversations(f.work.id)).toHaveLength(1);
    const queue = f.m.queues()[f.work.id]; expect(queue).toHaveLength(1);
    ...
    expect(f.m.projectHandoffs().list(f.work.id, first.target!.sessionId)[0].id).toBe(f.input.requestId);
  ...
  it('recovers after target creation without creating another target', () => {
    ...
    const target = f.m.listConversations(f.work.id)[0].sessionId; enqueue.mockRestore(); f.m.onModuleDestroy();
    ...
    expect(restarted.listConversations(f.work.id)).toHaveLength(1); expect(restarted.queues()[f.work.id]).toHaveLength(1);
  });
