...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('body').getAttribute('data-conversation-label-template'), 'static');
    const row = page.locator('.cr-task[data-session="' + c.sessionId + '"]');
    const visibleLabels = async () => row.evaluate((el) => [...el.querySelectorAll('.cr-row-title,.cr-row-subtitle')]
      .map((node) => ({ text: node.textContent.trim(), y: node.getBoundingClientRect().y }))
      .sort((a, b) => a.y - b.y));
    assert.deepEqual((await visibleLabels()).map((label) => label.text), [c.title, p.name]);
    ...
    assert.deepEqual((await visibleLabels()).map((label) => label.text), [p.name, c.title]);
