...
const shotDir = '/tmp/x056-control-room-check'; fs.mkdirSync(shotDir,{recursive:true});
(async () => {
 ...
 const settings=async(open,max)=>{await page.locator('#crSettings').click();await page.locator(`#displayPreferences input[name=open][value=${open}]`).check();await page.locator(`#displayPreferences input[name=maximize][value=${max}]`).check();await page.getByRole('button',{name:'Done',exact:true}).click();};
 ...
 await page.locator('#focusSettings').click();await page.locator('input[name=maximize][value=modal]').check();await page.getByRole('button',{name:'Done',exact:true}).click();await wait();
 assert.equal(await mode(),'modal');assert.equal(await page.locator('#focusNav').isVisible(),false);
 assert.equal(await page.locator('#chatDisplay').isVisible(),true);
 await page.screenshot({path:shotDir+'/modal-desktop.png'});
 await page.locator('#chatDisplay').click();await page.keyboard.press('Escape');assert.equal(await mode(),'modal');assert.equal(await page.locator('#displayPreferences').isVisible(),false);
 ...
 for(const width of [320,390,768,1440]) {
  ...
  const r=await page.locator('#sendBtn').boundingBox();assert.ok(r&&r.x>=0&&r.x+r.width<=width&&r.y+r.height<=900,`send visible at ${width}`);
  ...
  await page.locator('#crAccountsTab').click();await page.waitForFunction(()=>document.getElementById('accountExport')&&!document.getElementById('accountExport').disabled);await wait();
  ...
  await page.screenshot({path:shotDir+`/accounts-${width}.png`,fullPage:true});await page.locator('#crBoardTab').click();
 }
 await page.locator('#crAccountsTab').click();await page.waitForFunction(()=>!document.getElementById('accountExport').disabled);
 await page.locator('#accountProvider').selectOption('claude');await page.waitForFunction(()=>!document.getElementById('accountExport').disabled);assert.equal(await page.locator('.cr-account-row').count(),2);
 await page.locator('#accountDays').selectOption('30');await page.waitForFunction(()=>document.querySelectorAll('.cr-day').length===30);assert.equal(await page.locator('.cr-day').count(),30);
 await page.locator('[data-manage=primary]').click();await page.locator('[data-pause]').click();await page.waitForSelector('#accountDetail',{state:'detached'});await page.waitForTimeout(600);
 assert.match(await page.locator('.cr-account-row').filter({hasText:'primary@example.test'}).textContent(),/Paused/);
 await page.locator('[data-manage=primary]').click();await page.locator('[data-pause]').click();await page.waitForSelector('#accountDetail',{state:'detached'});
 const toggle=page.locator('[data-routing=claude]');await toggle.uncheck();await page.waitForTimeout(500);await page.reload();await page.waitForSelector('.cr-task');await page.locator('#crAccountsTab').click();await page.waitForSelector('[data-routing=claude]');assert.equal(await page.locator('[data-routing=claude]').isChecked(),false);await page.locator('[data-routing=claude]').check();
 ...
 await page.locator('#crBoardTab').click();await settings('side','page');await openCard('Review accessibility findings');
 ...
 await close();await page.locator('#crAccountsTab').click();await page.waitForFunction(()=>!document.getElementById('accountExport').disabled);
