...
(async()=>{const browser=await chromium.launch({args:['--no-sandbox']});try{
 ...
 await page.goto(base+'/projects/'+a.id+'/memory');await page.locator('#memoryProject').waitFor();assert.equal(await page.locator('#memoryProject').inputValue(),a.id);
 await page.locator('#memoryProject').selectOption(b.id);await page.locator('[data-memory-open="'+note.id+'"]').click();await page.locator('[data-memory-share]').click();await page.locator('dialog[open] select[name=recipient]').selectOption(a.id);await page.locator('dialog[open]').last().getByRole('button',{name:'Share',exact:true}).click();await page.locator('[data-revoke]').waitFor();
