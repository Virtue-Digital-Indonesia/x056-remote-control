...
(async () => {
 ...
 await page.locator('#moreBtn').click();assert.equal(await page.getByRole('menuitem',{name:'Results',exact:true}).isVisible(),true);assert.equal(await page.getByRole('menuitem',{name:'Message delivery',exact:true}).isVisible(),true);assert.equal(await page.getByRole('menuitem',{name:'Pin conversation',exact:true}).isVisible(),true);await page.keyboard.press('Escape');assert.equal(await mode(),'modal');
