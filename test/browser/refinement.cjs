...
(async () => {
 ...
 await page.locator('#manageRouting').click();const toggle=page.locator('[data-routing=claude]');await toggle.uncheck();await page.waitForTimeout(500);await page.reload();await page.waitForSelector('.cr-task');await nav('crAccountsTab');await page.locator('#manageRouting').click();await page.waitForSelector('[data-routing=claude]');assert.equal(await page.locator('[data-routing=claude]').isChecked(),false);await page.locator('[data-routing=claude]').check();await page.locator('[data-close-settings]').click();
