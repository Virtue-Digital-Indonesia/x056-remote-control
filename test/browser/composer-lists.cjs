...
(async () => {
  ...
  assert.equal(await prompt.inputValue(), '', 'Failed send stays in the saved outbox');
  await page.locator('#deliveryStrip button').click();
  assert.equal(await page.locator('.delivery-item p').innerText(), '- First\n  - Nested\n1. Numbered', 'Saved delivery preserves Markdown for retry');
