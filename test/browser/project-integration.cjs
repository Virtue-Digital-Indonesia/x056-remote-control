...
(async()=>{
 ...
 try{
  ...
  await page.locator('#rcProjectContext .rc-space-parent').click();await page.locator('.rc-space-tabs a').filter({hasText:'Files'}).click();await page.locator('#rcSpaceUploadInput').setInputFiles({name:'提案.txt',mimeType:'text/plain',buffer:Buffer.from('Saved integration original')});await page.locator('[data-project-file]').waitFor();await page.locator('.rc-space-file-menu').click();await page.getByRole('menuitem',{name:'Use in conversation',exact:true}).click();await page.locator('.rc-space-dialog [data-execution]').filter({hasText:'New Work in original'}).click();await page.waitForURL(base+workPath);assert.equal(await page.locator('#prompt').inputValue(),'Unsent Work integration draft');
