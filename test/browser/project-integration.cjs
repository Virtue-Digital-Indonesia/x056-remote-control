...
(async()=>{
 ...
 try{
  ...
  const workPath=new URL(page.url()).pathname;assert.equal(workPath.split('/')[2],other.id);await page.locator('#prompt').fill('Unsent Work integration draft');await page.locator('#rcProjectContext .rc-space-parent').waitFor();assert.match(await page.locator('#rcProjectContext').innerText(),/Research proposal/);
  await page.locator('#rcProjectContext .rc-space-parent').click();await page.locator('.rc-space-tabs a').filter({hasText:'Files'}).click();await page.locator('#rcSpaceUploadInput').setInputFiles({name:'提案.txt',mimeType:'text/plain',buffer:Buffer.from('Saved integration original')});await page.locator('[data-project-file]').waitFor();await page.locator('[data-project-file] [data-attach]').click();await page.locator('.rc-space-dialog [data-execution]').filter({hasText:'New Work in original'}).click();await page.waitForURL(base+workPath);assert.equal(await page.locator('#prompt').inputValue(),'Unsent Work integration draft');
  await page.reload();await page.locator('#rcProjectContext').waitFor();assert.equal(await page.locator('#prompt').inputValue(),'Unsent Work integration draft');
  ...
  await page.locator('#rcProjectContext [data-membership]').click();await page.locator('.rc-space-dialog select[name=destination]').selectOption(aId);await page.locator('[data-preview]').click();await page.locator('.rc-space-dialog .cr-primary:not([disabled])').click();await page.locator('.rc-space-dialog').waitFor({state:'detached'});assert.equal(new URL(page.url()).pathname,chatPath);assert.equal(await page.locator('#prompt').inputValue(),'Unsent Chat integration draft');
  ...
  await page.goto(base+'/projects/'+b.id+'/work');await page.locator('[data-work-project]').first().waitFor();await page.locator('#crProjectLinks a[href="/projects"]').waitFor();await page.screenshot({path:'/tmp/project-integration-desktop.png'});
  ...
  const loggedOut=await browser.newContext(),login=await loggedOut.newPage();login.on('pageerror',e=>errors.push(e.message));await login.goto(base+workPath);await login.locator('#gateToken').fill(token);await login.locator('#gateTokenBtn').click();await login.locator('#rcProjectContext').waitFor();assert.equal(new URL(login.url()).pathname,workPath);await loggedOut.close();
