...
(async()=>{
 ...
 try{
  ...
  await page.goto(base+'/projects/'+b.id+'/chat');await page.locator('#rcSpaceNewConversation').click();await page.locator('.rc-space-dialog select[name=provider]').selectOption('claude');await page.locator('.rc-space-dialog .cr-primary').click();await page.locator('#rcChatFiles').waitFor();const chatPath=new URL(page.url()).pathname;await page.locator('#prompt').fill('Unsent Chat integration draft');
