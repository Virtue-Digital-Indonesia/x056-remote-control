...
const fixtureRoot=process.env.X056_TEST_CHAT?resolve('.deploy/chat-browser'):tmpdir();mkdirSync(fixtureRoot,{recursive:true});
const dir=mkdtempSync(join(fixtureRoot,'x056-browser-')), state=join(dir,'state'), pub=join(dir,'public');
mkdirSync(state,{recursive:true});mkdirSync(pub);
for(const file of ['panel.html','control-room.js','control-room.css','rc-chat.js','rc-chat.css','webauthn.js','sw.js','manifest.webmanifest']) symlinkSync(file==='panel.html' && process.env.X056_TEST_PANEL ? process.env.X056_TEST_PANEL : resolve(['control-room.js','control-room.css'].includes(file) ? process.env.X056_TEST_ASSETS || 'server/public' : 'server/public',file),join(pub,file));
