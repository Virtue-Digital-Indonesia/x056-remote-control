...
for(const file of ['panel.html','control-room.js','control-room.css','webauthn.js','sw.js','manifest.webmanifest']) symlinkSync(file==='panel.html' && process.env.X056_TEST_PANEL ? process.env.X056_TEST_PANEL : resolve('server/public',file),join(pub,file));
...
writeFileSync(join(state,'questions.json'),JSON.stringify({[first]:{projectId:p.id,sessionId:first,question:'Which landing page should we publish?',options:['Main page','Campaign page'],at:new Date().toISOString()}}));
