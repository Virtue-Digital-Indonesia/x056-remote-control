...
const shots='/tmp/x056-refinement-scale-check';fs.mkdirSync(shots,{recursive:true});
(async()=>{
 ...
 const choose=async name=>{await page.locator('input[name=send-account][value='+name+']').check();await page.locator('[data-send-next]').click();await page.waitForSelector('#sendAccountPicker',{state:'hidden'});};
 ...
 await page.waitForFunction(()=>document.getElementById('sendAccountChip').textContent.includes('Personal workspace'));
 await page.locator('#sendAccountChip').click();
 assert.equal(await page.locator('input[name=send-account][value=chatgpt]').count(),0,'provider pools stay separate');
 await choose('backup');
 await page.waitForFunction(()=>document.getElementById('sendAccountChip').textContent.includes('Backup workspace'));
 assert.equal((await api('/api/accounts')).find(a=>a.nextUp&&a.provider==='claude').name,'backup');
 if(await page.locator('#sendAccountPicker').isVisible())await page.locator('#sendAccountPicker [data-close]').click();
 ...
 await page.waitForFunction(()=>document.getElementById('runningAccountLabel').textContent.includes('Working with Backup workspace'));
 ...
 await open('Review accessibility findings');
 await page.waitForFunction(()=>document.getElementById('runningAccountLabel').textContent.includes('Working with Backup workspace'));
 await page.locator('#sendAccountChip').click();await choose('primary');
 await page.waitForFunction(()=>document.getElementById('sendAccountChip').textContent.includes('Next message Personal workspace'));
 assert.match(await page.locator('#runningAccountLabel').textContent(),/Working with Backup workspace/);
 ...
 await page.locator('#sendAccountChip').click();await page.locator('input[name=send-account][value=primary]').check();await page.locator('[data-switch-turn]').click();
 await page.waitForFunction(()=>document.getElementById('runningAccountLabel').textContent.includes('Working with Personal workspace'),null,{timeout:10000});
 if(await page.locator('#sendAccountPicker').isVisible())await page.locator('#sendAccountPicker [data-close]').click();
 ...
 assert.equal(await page.locator('input[name=send-account][value=chatgpt]').count(),1);
 assert.equal(await page.locator('input[name=send-account][value=primary]').count(),0);
 ...
 await page.locator('#crTheme').click();await page.getByRole('menuitem',{name:'Dark',exact:true}).click();
