...
(async()=>{
 ...
 try{
  ...
  await page.locator('#crAccountsTab').click();await page.waitForURL(base+'/dashboard');await page.locator('#accountChart svg').waitFor();
  check('Dashboard selected',await page.locator('#crAccountsTab').getAttribute('aria-current')==='page');
  check('Dashboard charts and total retained',await page.locator('#accountModels').isVisible()&&await page.locator('#crProjectCosts').isVisible());
  await shot('dashboard-desktop');await page.reload();await page.locator('#accountChart svg').waitFor();check('Dashboard direct refresh',new URL(page.url()).pathname==='/dashboard');
  ...
  await page.locator('#crAccountsTab').click();await page.waitForURL(base+'/dashboard');await page.goBack();await page.locator('#prompt').waitFor();
  ...
  await page.locator('#crAccountsTab').click();await page.waitForURL(base+'/dashboard');check('Global navigation leaves embedded Memory',!await page.locator('#rcProjectPage').isVisible());
  ...
  const second=await context.newPage();await second.goto(base+'/dashboard');await second.locator('body.rc-workspace-ready').waitFor();await page.locator('[data-settings-page=general]').click();await page.locator('#stageVisible').uncheck();await second.waitForFunction(()=>document.body.dataset.stageHidden==='true');check('Switcher preference syncs across tabs',!await second.locator('#conversationStage').isVisible());await page.keyboard.press('Escape');await second.close();
  for(const path of ['/home','/activity','/accounts','/automations','/artifacts','/queue','/memory','/settings','/projects','/chat','/work/unassigned']){
  ...
  await go('/dashboard');check('Missing Chat state clears on navigation',!await page.locator('body').evaluate(e=>e.classList.contains('rc-chat-unavailable')));
  ...
  for(const width of [390,320,768]){
    ...
    if(width===390){
      ...
      await page.locator('#crProjects').click();await page.locator('#crAccountsTab').click();await page.waitForURL(base+'/dashboard');await page.locator('#accountChart svg').waitFor();await shot('dashboard-mobile');
  ...
  const signedOut=await browser.newContext(),login=await signedOut.newPage();login.on('pageerror',e=>errors.push(e.stack));await login.goto(base+'/dashboard');await login.locator('#gateToken').fill(token);await login.locator('#gateTokenBtn').click();await login.locator('#accountChart svg').waitFor();check('Sign in directly to Dashboard',new URL(login.url()).pathname==='/dashboard');await signedOut.close();
