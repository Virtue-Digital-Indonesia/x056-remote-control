...
(async()=>{
 ...
 try{
  ...
  await page.locator('#rcOverviewEditBrief').click();await page.locator('#rcBriefText').waitFor();
  check('Editing the brief starts from the existing text',(await page.locator('#rcBriefText').inputValue()).startsWith('Develop a hotel operations platform')&&await page.locator('dialog[open]').count()===0);
  await page.locator('#rcBriefText').fill('Updated approved hotel brief.');await page.locator('#rcBriefSave').click();await page.locator('#rcOverviewBrief').filter({hasText:'Updated approved hotel brief.'}).waitFor();
  ...
  for(const width of [390,320,768]){
    ...
    if(width===390){
      ...
      await page.locator((await page.locator('#menuBtn').isVisible())?'#menuBtn':'#crProjects').click();await page.locator('#crAccountsTab').click();await page.waitForURL(base+'/accounts');await page.locator('#accountChart svg').waitFor();await shot('accounts-mobile');
      ...
      await page.locator((await page.locator('#menuBtn').isVisible())?'#menuBtn':'#crProjects').click();await shot('navigation-mobile');await page.keyboard.press('Escape');check('Mobile drawer closes with Escape',!await page.locator('#controlRoom').evaluate(e=>e.classList.contains('projects-open')));await page.locator('#prompt').fill('Mobile draft retained.');await shot('chat-mobile');
    }
  }
